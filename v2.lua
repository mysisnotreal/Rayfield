...
-- Load local icons as primary source (Studio mode or fallback)
local Icons = nil
if useStudio then
	Icons = require(script.Parent.icons)
else
	-- Try to load tags.json for searching/fallback
	local loadTags = loadWithTimeout('https://cdn.jsdelivr.net/npm/lucide-static@latest/tags.json', 5)
	if loadTags and type(loadTags) == "table" then
		-- This is just for tag/search info, not for asset loading
	end
end
...
local function fetchAndCacheLucideIcon(iconName : string): string?
	...
	-- Try to fetch PNG from jsDelivr CDN (pre-rendered Lucide icons)
	local pngUrl = "https://cdn.jsdelivr.net/npm/lucide@latest/dist/esm/icons/" .. iconName .. ".js"
	...
	if not success or not response or not response.Body then
		-- Try alternative CDN with SVG
		if debugX then
			warn("Lucide Icons: Failed to fetch icon from primary CDN: " .. iconName)
		end
		return nil
	end
...
local function getIcon(name : string): string?
	...
	-- First try local Icons table (fastest, most reliable)
	if Icons then
	...
	-- Fallback: try to fetch and cache from network (if available)
	if getcustomasset and writefile and requestFunc then
	...
	if debugX then
		warn("Lucide Icons: Failed to find or fetch icon by the name of \"" .. name .. "\"")
	end
	return nil
end
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" then
		-- Try to fetch from local icons or return empty
		local assetId = getIcon(id)
		if assetId then
			assetUri = assetId
		end
	end
...
local function resolveIcon(icon)
	...
	if typeof(icon) == "string" then
		-- Try to get icon from dynamic GitHub-based system
		local assetId = getIcon(icon)
		if assetId then
