...
-- Dynamic icon system: fetches SVGs from Lucide GitHub repo, converts to PNG, and caches locally
local LucideGitHubURL = "https://raw.githubusercontent.com/lucide-icons/lucide/main/icons"
local LucideIconsCache = "Rayfield/LucideIcons"
local Icons = useStudio and require(script.Parent.icons) or nil

-- Cache for loaded icons to avoid repeated conversions
local iconCache = {}

local function ensureLucideCache()
	if isfolder and not callSafely(isfolder, LucideIconsCache) then
		callSafely(makefolder, LucideIconsCache)
	end
end

-- Fetch SVG from GitHub and convert to PNG, then cache it
local function fetchAndCacheLucideIcon(iconName : string): string?
	...
	local cacheFile = LucideIconsCache .. "/" .. iconName .. ".png"
	
	-- Check if file already exists locally
	if isfile and callSafely(isfile, cacheFile) then
		if getcustomasset then
			local assetId = callSafely(getcustomasset, cacheFile)
			if assetId then
				iconCache[iconName] = assetId
				return assetId
			end
		end
	end
	...
	local svgContent = loadWithTimeout(svgUrl, 10)
	
	if not svgContent or type(svgContent) ~= "string" or #svgContent == 0 then
		warn("Lucide Icons: Failed to fetch SVG for icon: " .. iconName .. " from " .. svgUrl)
		return nil
	end
	
	-- Convert SVG to PNG using online service (convertio.co API)
	-- This is a workaround since Lua doesn't have native SVG rendering
	-- Alternative: use a CDN that provides pre-rendered PNGs
	if not requestFunc then
		warn("Lucide Icons: Cannot convert SVG - no request function available")
		return nil
	end
	
	-- Use a simpler approach: fetch PNG from a CDN that provides Lucide icons
	local pngUrl = "https://cdn.jsdelivr.net/npm/lucide@latest/dist/png/48px/" .. iconName .. ".png"
	local pngData = nil
	
	-- Try to fetch PNG directly from CDN
	...
	if success and response and response.Body then
		...
		warn("Lucide Icons: Failed to fetch PNG for icon: " .. iconName)
		return nil
	end
	...
	ensureLucideCache()
	if writefile then
		callSafely(writefile, cacheFile, pngData)
	end
	
	-- Get custom asset ID and cache it
	if getcustomasset then
		local assetId = callSafely(getcustomasset, cacheFile)
		if assetId then
			iconCache[iconName] = assetId
...
local function getIcon(name : string): string?
	...
	-- Try to fetch and cache the icon from Lucide GitHub repo
	...
	-- Fallback to local Icons table if available (for Studio mode)
	if Icons then
		...
		if r then
			local rirs = r[2]
			local riro = r[3]
			if type(r[1]) == "number" and type(rirs) == "table" and type(riro) == "table" then
				return "rbxassetid://" .. r[1]
			end
		end
	end
	
	error("Lucide Icons: Failed to find or fetch icon by the name of \"" .. name .. "\"", 2)
end
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" then
		-- Try to fetch from GitHub Lucide repo
		local assetId = fetchAndCacheLucideIcon(id)
		if assetId then
			assetUri = assetId
		else
			warn("Rayfield | The icon argument must either be an icon ID (number) or a valid Lucide icon name (string)")
		end
	else
...
local function resolveIcon(icon)
	...
	if typeof(icon) == "string" then
		-- Try to get icon from dynamic GitHub-based system or fallback to getAssetUri
		local assetId = fetchAndCacheLucideIcon(icon)
		if assetId then
