...
-- Load icons from local file (1700+ Lucide icons)
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://cdn.jsdelivr.net/npm/lucide@latest/dist/icons/assets.json', 5)
...
local function getIcon(name : string): {id: number, imageRectSize: Vector2, imageRectOffset: Vector2}
	...
	if type(r[1]) ~= "number" or type(rirs) ~= "table" or type(riro) ~= "table" then
		warn("Lucide Icons: Internal error: Invalid auto-generated asset entry for \"" .. name .. "\"")
		return nil
	end
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and Icons then
		...
		if asset then
			assetUri = "rbxassetid://" .. asset.id
		else
			warn("Rayfield | Cannot find Lucide icon: " .. id)
		end
	else
