...
-- Fetch raw content from URL without executing (for SVG files, etc.)
local function fetchRawContent(url: string, timeout: number?): string?
	assert(type(url) == "string", "Expected string, got " .. type(url))
	timeout = timeout or 5
	local requestCompleted = false
	local success, result = false, nil
	...
	local requestThread = task.spawn(function()
		...
		if fetchSuccess and fetchResult and #fetchResult > 0 then
			success, result = true, fetchResult
		else
...
-- Fetch icons from GitHub Lucide repo, cache with writefile, and use getcustomasset
local LucideGitHubURL = "https://raw.githubusercontent.com/lucide-icons/lucide/main/icons"
local LucideIconsFolder = "Rayfield/LucideIcons"

-- Cache for loaded icon asset IDs
local iconAssetCache = {}

local function ensureLucideFolder()
	if isfolder and not callSafely(isfolder, LucideIconsFolder) then
		callSafely(makefolder, LucideIconsFolder)
	end
end

-- Fetch icon SVG from GitHub, convert to PNG, cache it, and get asset ID
local function fetchAndCacheLucideIcon(iconName : string): string?
	if not iconName or type(iconName) ~= "string" then
		return nil
	end
	
	iconName = string.match(string.lower(iconName), "^%s*(.*)%s*$") :: string
	
	-- Check if already cached in memory
	if iconAssetCache[iconName] then
		return iconAssetCache[iconName]
	end
	
	-- Ensure folder exists
	ensureLucideFolder()
	
	local iconPngPath = LucideIconsFolder .. "/" .. iconName .. ".png"
	
	-- Check if PNG already exists in cache
	if isfile and callSafely(isfile, iconPngPath) then
		if getcustomasset then
			local assetId = callSafely(getcustomasset, iconPngPath)
			if assetId then
				iconAssetCache[iconName] = assetId
				return assetId
			end
		end
	end
	
	-- Fetch SVG from GitHub Lucide repo
	local svgUrl = LucideGitHubURL .. "/" .. iconName .. ".svg"
	local svgContent = fetchRawContent(svgUrl, 10)
	
	if not svgContent or type(svgContent) ~= "string" or #svgContent == 0 then
		if debugX then
			warn("Lucide Icons: Failed to fetch SVG: " .. iconName)
		end
		return nil
	end
	
	-- Convert SVG to PNG using online API
	local pngData = nil
	
	-- Use svgtopng.netlify.app API to convert SVG to PNG
	if requestFunc and HttpService then
		local success, response = pcall(function()
			return requestFunc({
				Url = "https://svgtopng.netlify.app/api/convert",
				Method = "POST",
				Headers = {
		...
		if success and response and response.Body and #response.Body > 0 then
			...
			if debugX then
				print("Lucide: Converted " .. iconName .. " SVG to PNG (" .. #pngData .. " bytes)")
			end
		end
	end
	
	-- If conversion failed, try alternative: fetch from a cached PNG CDN
	if not pngData then
		-- Try to fetch pre-rendered PNG from jsDelivr (if available for this icon)
		local pngCdnUrl = "https://cdn.jsdelivr.net/npm/@lucide/static@latest/48px/" .. iconName .. ".png"
		local cdnSuccess, cdnResponse = pcall(function()
		...
		if cdnSuccess and cdnResponse and cdnResponse.Body then
			pngData = cdnResponse.Body
		end
	end
	
	-- Final fallback: Save SVG as PNG file (Roblox may still be able to use it)
	if not pngData then
		pngData = svgContent
		if debugX then
			warn("Lucide: Using SVG directly as " .. iconName .. ".png (conversion failed)")
		end
	end
	
	if not pngData or #pngData == 0 then
		if debugX then
			warn("Lucide Icons: Failed to process icon: " .. iconName)
		end
		return nil
	end
	...
	if writefile then
		callSafely(writefile, iconPngPath, pngData)
	end
	
	-- Get custom asset ID from the saved file
	if getcustomasset then
		local assetId = callSafely(getcustomasset, iconPngPath)
		if assetId then
			iconAssetCache[iconName] = assetId
			if debugX then
				print("Lucide: Cached " .. iconName .. " as custom asset")
			end
			return assetId
		end
	end
...
-- Load icons - use local icons for Studio, or fetch from GitHub for runtime
...
local function getIcon(name : string): string?
	...
	-- First try local Icons table (if available - Studio mode)
	if Icons then
		local sizedicons = Icons['48px']
		if sizedicons then
			local r = sizedicons[name]
			if r and type(r[1]) == "number" then
				return "rbxassetid://" .. r[1]
			end
		end
	end
	
	-- Fallback: fetch from GitHub Lucide repo
	if getcustomasset and writefile and loadWithTimeout then
		local assetId = fetchAndCacheLucideIcon(name)
		if assetId then
			return assetId
		end
	end
	...
	if debugX then
		warn("Lucide Icons: Failed to find or fetch icon: " .. name)
	end
	return nil
end
...
local function resolveIcon(icon)
	...
	if typeof(icon) == "string" then
		local assetUri = getIcon(icon)
		if assetUri then
			return assetUri, nil, nil
		end
		return "", nil, nil
	else
